...
(def code
  (let [jarpath "target/app.jar"
    ...
    (with-out-str
      ...
      (compile 'users.client)
      (compile 'users.lambda)
      (uberdeps/package deps jarpath {:aliases [:classes]}))
    (Code/fromAsset jarpath)))

(def app (App))

(def stack (Stack app "generate-user-lambda"))

(def bucket (Bucket stack "generate-user-lambda-bucket"))

(def my-fn
  (Function stack
            "send-topup-fn"
            {:code        code        ;; Calling a static method
             :handler     "com.anudis.hmrc.sandbox.GenerateUser"
